 ...
 * Both the media type and sub type can also be the wild card <code>*</code> such as <code>*&#47;*</code> and <code>text&#47;*</code>. Note, if the media type is the wild card
...
public class MimeType implements Comparable, Serializable {
	...
	public boolean equals(Object o) {
		if (o instanceof MimeType) {
			if (mediaType.equals(((MimeType) o).mediaType) && subType.equals(((MimeType) o).subType)) {
				return true;
			}
	 ...
	 * <p>
	 * This is calculated by the number of times this MimeType would be returned if the Collection was not normalised. The higher the count the more MimeDetectors have matched this type. As this can
	 * be a false positive for types such as application/octect-stream and text/plain where they would be returned by multiple MimeDetector(s). These types are referred to as root mime types where ALL
	 * mime types derive from application/octet-stream and all text/* types derive from text/plan so in these cases we set the specificity to 0 no matter how many times they match. This ensures they
	 * are regarded as the least specific in the returned Collection.
	 * </p>
	 ...
	 * T O D O: Enforce more rigorous checking of valid media types.
	 */
	 ...
	 * T O D O: Enforce more rigorous checking of valid sub types.
	 */
